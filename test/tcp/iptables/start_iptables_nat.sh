...
iptables -t nat -A PROXY_INIT_REDIREC -p tcp -m owner --uid-owner 0 -m comment --comment "ignore root package" -j RETURN
...
iptables -t nat -A PROXY_INIT_OUTPUT -p tcp -m multiport --dports 5000 -j PROXY_INIT_REDIREC #在端口为80时执行PROXY_INIT_REDIREC链